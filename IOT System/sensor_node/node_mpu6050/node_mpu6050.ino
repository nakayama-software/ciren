...
void loop() {
  ...
  // Kirim data ke controller setiap SEND_INTERVAL_MS (20Hz)
  // Filter tetap jalan 100Hz untuk akurasi complementary filter
  if (now_ms - last_send_ms >= SEND_INTERVAL_MS) {
    last_send_ms = now_ms;
    last_hb_ms   = now_ms;  // reset HB timer setiap data terkirim
    ...
    // (relevan jika data interval diperlambat via config)
    last_hb_ms = now_ms;
    uint32_t shared_ts = millis();
