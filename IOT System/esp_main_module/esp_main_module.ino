...
// RASPI SERIAL SECTION -- STARTS HERE
String serialRxBuf = "";
...
bool isValidPiSerial(const String &s)
{
  if (s.length() != 16)
    return false;
...
void handleSerialLine(String line)
{
  ...
  else if (line.startsWith("PI_SERIAL:"))
  {
    String id = line.substring(strlen("PI_SERIAL:"));
    id.trim();

    if (isValidPiSerial(id))
    {
      id.toUpperCase();
    ...
    {
      Serial.print("[WARN] Invalid PI serial: ");
      Serial.println(id);
    }
  }
  else
  {
    Serial.print("[INFO] Ignored host line: ");
    Serial.println(line);
  }
