...
export default function ThresholdModal({ open, onClose, deviceId, ctrlId, portNum, sensorType }) {
  ...
  useEffect(() => {
    if (!open) return
    const t = getThreshold(deviceId, ctrlId, portNum, sensorType)
    setMinVal(t.min != null ? String(t.min) : '')
    setMaxVal(t.max != null ? String(t.max) : '')
      ...
      <div className="w-full max-w-sm rounded-2xl border border-black/10 bg-white dark:bg-slate-900 dark:border-white/10 shadow-xl">
        <div className="flex items-center justify-between px-4 py-3 border-b border-black/10 dark:border-white/10">
          <div className="flex items-center gap-2 text-sm font-medium text-slate-900 dark:text-white">
            ...
            <span>Threshold — {info.label}</span>
          </div>
        ...
        <div className="p-4 space-y-3">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Alert when value goes outside these bounds. Leave blank to skip that side.
          </p>
          ...
          <p className="text-[11px] text-gray-400 dark:text-gray-600">
            Ctrl {ctrlId} / Port {portNum} · stored locally per browser
          </p>
        </div>
