...
function useSensorFeed(deviceId, ctrlId, portNum, sensorType, open, wsRef) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const data = await getHistory(deviceId, ctrlId, portNum, 1, sensorType)
        if (cancelled) return
  ...
  return (
    <div className="rounded-xl border border-black/10 dark:border-white/10 bg-white dark:bg-slate-800 p-4 flex flex-col" style={{ minHeight: '260px' }}>
      <div className="flex items-start justify-between mb-3 flex-shrink-0">
        <div>
          <span className="text-[10px] font-medium uppercase tracking-wider text-gray-400 dark:text-gray-500">Port {portNum}</span>
...
// ─── Main modal ───────────────────────────────────────────────────────────────
export default function MultiSensorView({ open, onClose, label, deviceId, ctrlId, wsRef, onPopOut, latestData }) {
      ...
      <div className="relative w-full max-w-5xl max-h-[90vh] flex flex-col rounded-2xl border border-black/10 dark:border-white/10 bg-slate-50 dark:bg-slate-900 shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className={`grid ${gridClass} gap-4`}>
            {sensors.map((sensor) => (
              <SensorPanel
                key={`${sensor.portNum}-${sensor.sensorType}`}
                deviceId={deviceId}
                ctrlId={ctrlId}
                sensor={sensor}
                open={open}
