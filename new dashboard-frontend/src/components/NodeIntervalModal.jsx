...
export default function NodeIntervalModal({ open, onClose, deviceId, ctrlId, portNum, currentIntervalMs, t }) {
  ...
  async function handleSave() {
    ...
    setError(null)
    try {
      await setNodeConfig(deviceId, Number(ctrlId), Number(portNum), ms)
      onClose(ms)
    } catch (e) {
      setError(tr.error || 'Failed to save')
      setSaving(false)
    }
  }
        ...
        <div className="p-4 space-y-3">
          ...
          {currentIntervalMs != null && (
            <p className="text-xs text-cyan-600 dark:text-cyan-400">
              {tr.current ? tr.current(currentIntervalMs) : `Current: ${currentIntervalMs}ms`}
            </p>
          )}
