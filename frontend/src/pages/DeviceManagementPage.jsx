    ...
    <div lang={t.locale}
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-5 py-5">
          ...
          <div className="relative flex sm:hidden">
            ...
            {mobileMenuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 z-50 w-52 rounded-2xl border border-black/10 bg-white shadow-xl dark:border-white/10 dark:bg-slate-900 overflow-hidden">
                  {/* Theme */}
                  <button type="button" onClick={() => { toggleTheme(); setMobileMenuOpen(false) }}
                    ...
                    {theme === 'dark' ? <Sun className="w-4 h-4 text-yellow-400 shrink-0" /> : <Moon className="w-4 h-4 text-indigo-500 shrink-0" />}
                    ...
                    <div className="flex gap-1">
                      {['ja', 'en'].map((lang) => (
                        <button key={lang} type="button" onClick={() => { setLanguage(lang); setMobileMenuOpen(false) }}
                          className={`px-2.5 py-1 text-xs rounded-md cursor-pointer transition-colors ${language === lang
                            ? (theme === 'dark' ? 'bg-white text-slate-900' : 'bg-slate-900 text-white')
        ...
        <div className="rounded-2xl border border-black/10 bg-white/80 p-4 sm:p-5 backdrop-blur-sm dark:border-white/10 dark:bg-slate-800/60 shadow-sm mb-6">
          ...
          <p className="text-xs text-slate-500 dark:text-gray-400 mb-3 sm:mb-4">
            {language === 'ja'
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:items-end">
            ...
            <button type="button" onClick={handleAdd} disabled={adding}
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-gray-100 disabled:opacity-50 cursor-pointer shrink-0">
              ...
              <div key={id}
                ...
                {deletingId === id ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        <button onClick={() => handleDelete(id)}
                          className="inline-flex items-center justify-center gap-1 rounded-lg bg-red-600 px-3 py-2 text-xs font-medium text-white hover:bg-red-700 cursor-pointer">
                          <Trash2 className="w-3 h-3" /> {language === 'ja' ? '削除する' : 'Delete'}
                        </button>
                        <button onClick={() => setDeletingId(null)}
                          className="inline-flex items-center justify-center gap-1 rounded-lg border border-black/10 px-3 py-2 text-xs font-medium text-slate-900 hover:bg-black/5 dark:border-white/10 dark:text-white dark:hover:bg-white/10 cursor-pointer">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 sm:h-10 sm:w-10 shrink-0 items-center justify-center rounded-lg bg-gradient-to-r from-cyan-500 to-indigo-500">
                      <Cpu className="h-4 w-4 sm:h-5 sm:w-5 text-white" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slate-900 dark:text-white font-mono truncate">{id}</p>
