...
export default function CurrentCard({ portNum, reading, status, onChartClick }) {
  const value = reading?.value != null ? Number(reading.value) : null
  const display = value !== null ? value.toFixed(3) : '--'
  const pct = value !== null ? Math.min(100, Math.max(0, (value / MAX_CURRENT) * 100)) : 0
  const barColor = pct > 80 ? 'bg-red-500' : pct > 50 ? 'bg-orange-400' : 'bg-orange-500'
  ...
  return (
    <div
      ...
      className="rounded-xl border border-orange-500/20 bg-orange-500/10 p-4 hover:border-orange-500/40 transition-all cursor-pointer h-full flex flex-col"
    >
      <div className="flex items-center gap-3">
        <div className="rounded-lg bg-orange-500/10 border border-orange-500/20 p-2 shrink-0">
          <Activity className="w-5 h-5 text-orange-500 dark:text-orange-300" />
        </div>
        <div className="min-w-0 flex-1">
          <p className="font-semibold text-slate-900 dark:text-white text-sm">Current</p>
      ...
      <div className="rounded-xl bg-slate-100/80 dark:bg-black/20 border border-slate-200 dark:border-white/10 p-3 mt-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="text-right">
            ...
            {value !== null && <p className="text-[11px] text-slate-400 dark:text-gray-500 mt-1">A</p>}
          </div>
        </div>
      </div>
