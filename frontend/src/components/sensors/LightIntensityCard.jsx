...
export default function LightIntensityCard({ portNum, reading, status, onChartClick }) {
  ...
  return (
    <div
      ...
      className="rounded-xl border border-amber-400/20 bg-amber-400/10 p-4 hover:border-amber-400/40 transition-all cursor-pointer h-full flex flex-col"
    >
      <div className="flex items-center gap-3">
        <div className="rounded-lg bg-amber-400/10 border border-amber-400/20 p-2 shrink-0">
          <Sun className="w-5 h-5 text-amber-400" />
        </div>
        <div className="min-w-0 flex-1">
          <p className="font-semibold text-slate-900 dark:text-white text-sm">Light Intensity</p>
      ...
      <div className="rounded-xl bg-slate-100/80 dark:bg-black/20 border border-slate-200 dark:border-white/10 p-3 mt-3 flex-1 flex flex-col justify-center">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="text-right">
            ...
            {value !== null && <p className="text-[11px] text-slate-400 dark:text-gray-500 mt-1">lux</p>}
          </div>
        </div>
      </div>
    </div>
