...
SER_HANDLE: Optional[serial.Serial] = None  # untuk ESP32 receiver
...
def detect_serial_port() -> Optional[str]:
    ...
    Abaikan port GPS (CFG.gps_port) agar tidak bentrok."""
    gps_dev = CFG.gps_port.lower()
    for p in list_ports.comports():
        dev = p.device.lower()
        # skip GPS port so auto-detect never grabs it
        if dev == gps_dev:
            continue
...
def parse_cgpsinfo(line: str) -> Dict[str, Any]:
    ...
    # SIMCom time comes from fields; sebagian modul kirim UTC; jika kosong, gunakan now
    ts_iso = datetime.utcnow().isoformat() + "Z"
...
def worker_gps_reader():
    """GPS Reader: init GNSS (AT+CFUN=1, AT+CGPS=1) dan polling AT+CGPSINFO.
    Berjalan di port terpisah (CFG.gps_port)."""
    port = CFG.gps_port
    baud = CFG.baud_rate
    ...
    def _send(ser: serial.Serial, cmd: str, wait: float = 0.4) -> str:
        try:
    ...
    while not STOP.is_set():
        ...
        try:
            ...
            for line in resp.splitlines():
                ...
                if st.get("status") == "fix":
                    if last_status != "fix":
                        log.info("[GPS] Fix acquired")
                    last_status = "fix"
                    payload = {
                        "sensor_controller_id": "GPS_DATA",
                        "lat": st["lat"],
                        "lon": st["lon"],
                        "alt_m": st.get("alt_m"),
                        "speed_knots": st.get("speed_knots"),
                        "ts_iso": st.get("ts_iso"),
                    }
                    queue_put(payload)
                    log.info(f"[GPS] {st['lat']:.8f}, {st['lon']:.8f}")
                elif st.get("status") == "nofix":
                    now = time.time()
                    if last_status != "nofix" or (now - last_warn_ts) > WARN_COOLDOWN:
                        log.warning("[GPS] No fix — waiting for satellites…")
                        last_warn_ts = now
                        # Optional: also push NO_FIX status to server
                        queue_put({
                            "sensor_controller_id": "GPS_STATUS",
                            "status": "NO_FIX",
                            "ts_iso": datetime.utcnow().isoformat() + "Z",
                        })
                    last_status = "nofix"
...
def main() -> int:
    ...
    log.info(f"[CFG] GPS_PORT → {CFG.gps_port}")
