...
function parseRotary(node) {
  const raw = node?.sensor_data ?? node?.value ?? node?.readings?.[0]?.value ?? "";
  const s = String(raw ?? "");
  let direction = null;
  let steps = null;
    ...
    return (
      <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 p-4 hover:bg-slate-100 dark:hover:bg-white/10 transition">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <p className="font-semibold text-slate-900 dark:text-white">Rotary Sensor</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <MetricTile
            icon={<RotateCw className="w-4 h-4 text-amber-500 dark:text-amber-300" />}
            label="Direction"
          ...
          />
          <MetricTile
            icon={<span className="text-amber-500 dark:text-amber-300 text-xs font-bold tabular-nums">Δ</span>}
            label="Steps / Count"
            value={fmt(steps, 0)}
            unit={node?.unit || "step"}
            accent="amber"
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 p-4 hover:bg-slate-100 dark:hover:bg-white/10 transition">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right">
          <p className="text-2xl font-mono text-slate-900 dark:text-white tabular-nums leading-none">{fmt(steps, 0)}</p>
          <p className="text-[11px] text-slate-400 dark:text-gray-500 mt-1">{node?.unit || "step"}</p>
        </div>
      </div>
      <div className="mt-3 text-[11px] text-slate-500 dark:text-gray-400">
        Direction: <span className="text-slate-800 dark:text-gray-200 font-medium">{directionLabel}</span>
      </div>
    </div>
  );
}