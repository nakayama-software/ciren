...
function buildIMUFromNode(node) {
  ...
  if (imu?.accel && imu?.gyro) {
    return { accelerometer: imu.accel, gyroscope: imu.gyro, temperature: typeof imu.tempC === "number" ? imu.tempC : null };
  }
  ...
  const fallbackRaw = (typeof node?.sensor_data === "string" && node.sensor_data) || (typeof node?.value === "string" && node.value) || "";
