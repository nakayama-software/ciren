...
function Register() {
  ...
  useEffect(() => {
    if (isLoggedIn()) navigate('/ciren/raspis', { replace: true });
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    if (password.length < 6) {
      setErrorMsg(t.alerts.passwordTooShort);
      return;
    }
    if (password !== confirmPassword) {
      setErrorMsg(t.alerts.passwordMismatch);
      return;
    }
  ...
  return (
    <div lang={t.locale}
      className="fixed inset-0 min-h-screen overflow-hidden font-['Noto_Sans_JP','Hiragino Kaku Gothic ProN','Yu Gothic UI',system-ui,sans-serif]
                 selection:bg-cyan-300/30 selection:text-white bg-slate-50 text-slate-900
                 dark:text-white dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 transition-colors duration-500">
          ...
          <div className="w-full max-w-md">
            <div className="rounded-2xl border border-black/10 bg-white/80 p-6 backdrop-blur-sm dark:border-white/10 dark:bg-slate-800/60 shadow-sm">
              ...
              <div className="space-y-4">
                {/* Username */}
                <div>
                  <label htmlFor="reg-username" className="block text-xs text-gray-700 dark:text-gray-300 mb-2">
                    {t.usernameLabel}
                  </label>
                ...
                {/* Password */}
                <div>
                  <label htmlFor="reg-password" className="block text-xs text-gray-700 dark:text-gray-300 mb-2">
                    {t.passwordLabel}
                  </label>
                ...
                {/* Confirm Password */}
                <div>
                  <label htmlFor="reg-confirm" className="block text-xs text-gray-700 dark:text-gray-300 mb-2">
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button type="button" onClick={handleSubmit} disabled={loading}
                    className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 focus:ring-2 focus:ring-cyan-400 dark:bg-white dark:text-slate-900 dark:hover:bg-gray-100 disabled:opacity-50">
                    {loading ? (language === 'ja' ? '作成中...' : 'Creating...') : t.registerButton}
                    {!loading && <ArrowRight className="h-4 w-4" />}
                  </button>
                  <button type="button" onClick={() => navigate('/ciren')}
